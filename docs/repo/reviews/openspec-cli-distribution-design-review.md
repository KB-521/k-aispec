...
| --- | --- | --- | --- | --- | --- |
| 中 | 首版默认 profile 范围是否仅限 `.codex/skills/` + `openspec/` | 影响实现范围和兼容成本 | 明确是否需要同时兼容 `.agents/skills/`、`.claude/skills/` | resolved | 已纳入默认分发范围 |
| 中 | 包内资产目录是直接发布原始路径还是生成 `distribution/managed/` | 影响打包脚本和运行时路径解析 | 建议统一生成 `distribution/managed/`，避免把仓库工程文件一并暴露为分发资产 | resolved | 已按独立分发目录实现 |
